...
export const StatusBadge = ({ status, avoidDanger = false, uppercase = false }) => {
  const map = {
  ...
  if (uppercase) {
    return <span className="ui-badge" style={{ background: item.bg, color: item.c, fontWeight: 700, padding: "4px 11px", textTransform: "uppercase" }}>{item.t}</span>;
  }
