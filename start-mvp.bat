...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$root = (Resolve-Path '%~dp0').Path; Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -and $_.CommandLine.Contains($root) -and ($_.CommandLine -like '*server\index.mjs*' -or $_.CommandLine -like '*vite*bin*vite.js*') } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }"
echo.
