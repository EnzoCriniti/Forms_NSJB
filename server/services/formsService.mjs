/**
 * @file server/services/formsService.mjs
...
const normalizeResultsConfig = config => {
...
export const saveForm = payload => {
  ...
  const duplicate = findConflictingFormBySlug(slug, payload.id);
  if (duplicate) throw new Error("Ja existe um formulario com este slug.");

  const existingForm = payload.id ? findFormById(payload.id) : null;
  ...
  const values = {
    ...
    resultsConfig: normalizeResultsConfig(payload.resultsConfig),
  ...
  const formId = upsertFormRecord(values);

  if (values.type === "escala_organ") {
    const currentSections = getEscalaByFormId(formId);
    upsertEscalaRecord(formId, currentSections.length ? currentSections : buildScaleSections(values.scaleSections));
  }

  return findFormById(formId);
};

export const deleteForm = (formId, masterKey) => {
  ...
  return withTransaction(() => {
    ...
    const keyStatus = getFormDeleteKeyStatus();
    ...
    if (!verifyFormDeleteKey(masterKey)) {
      throw makeError("Chave mestra incorreta.", 403, "MASTER_KEY_INVALID");
    }

    deleteFormRecordWithDependencies(formId);
    return { ok: true };
  });
};
