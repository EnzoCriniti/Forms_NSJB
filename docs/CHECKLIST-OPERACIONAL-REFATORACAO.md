...
- iniciado neste ciclo: normalizacao do bootstrap saiu para `frontend/src/lib/appBootstrapNormalize.js` e selecao ativa para `frontend/src/lib/appBootstrapSelection.js`; `appBootstrap.js` ficou como agregador historico.
...
- iniciado neste ciclo: estado local, derived state, view model e composicao dos handlers sairam para `frontend/src/screens/createFormController.js`; `CreateFormScreen.jsx` ficou como composicao visual dos paineis.
- iniciado neste ciclo: a fronteira entre `createFormController.js` e `CreateFormPresenceSection.jsx` passou a usar blocos nomeados de props (`fieldsPanel`, `fieldEditor`, `resultsConfig`) em vez de uma lista plana extensa.
- pendente apos este corte: `fieldEditor` ainda e o maior bloco; proximo corte so deve mexer nele se houver ganho claro em dividir draft, catalogo, grid e acoes sem espalhar regra visual.
...
- iniciado neste ciclo: toolbar de filtros, zoom e exportacao da presenca saiu para `frontend/src/screens/PresenceResultsToolbar.jsx`.
...
- iniciado neste ciclo: adapters por aba da central administrativa foram divididos por dominio em `adminSettingsAccessTabAdapters.js`, `adminSettingsMemberTabAdapters.js`, `adminSettingsCatalogTabAdapters.js`, `adminSettingsOrganizationTabAdapters.js`, `adminSettingsMessagingTabAdapters.js`, `adminSettingsSecurityTabAdapters.js` e `adminSettingsAuditTabAdapters.js`.
...
- iniciado neste ciclo: `adminFieldPreview.jsx` foi dividido em blocos especificos para `person_select` e grade, reduzindo os ramos grandes de renderizacao.
- iniciado neste ciclo: filtros da auditoria sairam para `AuditLogsFilterFields.jsx` e `AuditLogsFiltersActions.jsx`; `AuditLogsFiltersPanel.jsx` ficou como composicao fina.
- iniciado neste ciclo: barra de resumo, barra de paginacao e tabela da auditoria sairam para `AuditLogsSummaryBar.jsx`, `AuditLogsPaginationBar.jsx` e `AuditLogsTable.jsx`; `AuditLogsPanel.jsx` ficou como composicao fina.
...
- iniciado neste ciclo: estado, submit e confirmacao do bloco de modelos sairam para `useMessagingTemplatesController.js`.
- iniciado neste ciclo: estado, filtro, ordenacao e confirmacao do bloco de presets sairam para `useMessagingPresetsController.js`.
...
- iniciado neste ciclo: `CatalogManagementModal.jsx` foi dividido em `CatalogManagementLabelsList.jsx` e `CatalogManagementPresetsList.jsx`, deixando o modal legado como composicao fina.
- iniciado neste ciclo: `adminSecurityPanels.jsx` foi dividido em `SecurityKeyFormPanel.jsx` e `SecurityStatusPanel.jsx`, deixando o painel compartilhado mais fino.
