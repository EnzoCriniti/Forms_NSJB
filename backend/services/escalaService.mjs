/**
 * @file backend/services/escalaService.mjs
...
import { getScalePersonLimit } from "../../shared/formRules.mjs";
...
const ensureScaleForm = async formId => {
...
const getScaleBaseSections = async formId => {
...
export const saveEscala = async (formId, sections) => {
  const form = await ensureScaleForm(formId);
  validatePersonAssignmentLimit(sections, getScalePersonLimit(form));

  return database.withTransaction(async () => {
    await upsertEscalaRecord(formId, sections);
    return getEscalaByFormId(formId);
  });
};

export const claimEscalaSlot = async (formId, sectionIndex, slotIndex, person) => {
  const form = await ensureScaleForm(formId);
  const limit = getScalePersonLimit(form);
  const nextPerson = cleanPerson(person);

  return database.withTransaction(async () => {
    const currentSections = await getScaleBaseSections(formId);
    ...
    await upsertEscalaRecord(formId, nextSections);
    return getEscalaByFormId(formId);
  });
};
