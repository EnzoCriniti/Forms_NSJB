/**
 * @file backend/services/responsesService.mjs
...
import { validateResponseValuesAgainstForm } from "../../shared/formRules.mjs";
...
export const saveResponse = async payload => {
  ...
  const form = await findFormById(payload.formId);
  ...
  if (duplicateBlocked) {
    ...
    const existingResponse = (await listResponsesByFormId(payload.formId)).find(response => String(response.respondentName || "").trim().toLowerCase() === normalizedRespondentName);
  ...
  const result = await upsertResponseRecord({
  ...
  return {
    responses: await listResponsesByFormId(payload.formId),
